...
int udp_sendmsg(struct sock *sk, struct msghdr *msg, int len)
{
	...
	/* beginning of segment added by Allen Downey to detect
	   outgoing clink datagrams and timestamp them */

	struct iovec *msg_iov = msg->msg_iov;
	char *buff = msg_iov[0].iov_base;
	int iov_len = msg_iov[0].iov_len;

	int i, count = 0;
	char *match = "<clink!>";
	int match_len = 8;
	int complen = (iov_len < match_len) ? iov_len : match_len;

	for (i=0; i<complen; i++) {
	  if (buff[i] != match[i]) break;
	  count++;
	}
	if (count == 8) {
	  struct timeval *tv = (struct timeval *)buff;
	  do_gettimeofday(tv);
	  /* printk (KERN_ERR "Sending special UDP datagram\n"); */
	}

	/* end of segment added by Allen Downey */
