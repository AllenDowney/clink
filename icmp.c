...
/* play_that_funky_music: added by Allen Downey to detect incoming
   ICMP packets that resulted from clink datagrams */   

static void play_that_funky_music (struct icmphdr *icmph)
{
  struct timeval *tv;
  struct iphdr *iph = (struct iphdr *) (icmph + 1);

  if ((iph->tos & IPTOS_TOS_MASK) != 
      (IPTOS_LOWDELAY | IPTOS_THROUGHPUT | IPTOS_MINCOST)) return;

  tv = (struct timeval *) &iph->saddr;
  do_gettimeofday (tv);
  /* printk (KERN_ERR "mangled ICMP packet\n"); */
}
